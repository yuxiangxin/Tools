...
::*************************配置项*****************
::配置目标package
set package=com.yu.litepaltest
...
:ff3 (导出文件)
adb -s %selectDevices%  shell find data/data/%package% -type f >%lsfile%
...
:ff4 (导入文件)
adb -s %selectDevices%  shell find data/data/%package% -type d >%lsfile%
...
adb -s %selectDevices%  shell find data/data/%package% -type f %resilt% >%lsfile%
set filter=
call :select file %LIST_FILE%
echo 删除文件:%file%
adb root
...
pause&goto:main
:getSelect
adb -s %selectDevices%  shell find data/data/%package% -type f -name *.%1 >%lsfile%
